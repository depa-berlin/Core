...
trait DatabasePersistenceTrait 
{
	...
	*/
	...
	public static function find($condition)
	{
	    ...
	    {
	        $condition = array(self::getPrimaryKeys()[0] => $condition);
	    }
	...
	public static function getRecordCount($condition = NULL)
	{
	    ...
	    $select = new Select();
	    $select->from(static::$_configuration[get_called_class()]['tablename'])->columns( ['recordcount' => new \Zend\Db\Sql\Expression('COUNT(*)')] );
	    if ($condition !== NULL)
	    {
	        $select->where($condition);
	    }
