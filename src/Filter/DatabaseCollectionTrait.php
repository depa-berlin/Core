...
trait DatabaseCollectionTrait {
    ...
    public function saveFilters($filterableObject)
    {
        ...
        $sqlQuery .= ";";
        return $this->adapter->query($sqlQuery);
    }
    ...
    public function loadFilters($filterableObject)
    {
        ...
        return $this->adapter->fetchAll($sqlQuery);
    }
     ...
     */
    public function loadIdsByQuery(\Depa\Core\Filter\Query $query)
    {
        $result = array();
        foreach ($query->getFilterables() as $filterable)
        {
            ...
            foreach ($query->getParams() as $categoryId => $filterArray)
            {
                foreach ($filterArray as $filter)
                {
                    ...
                    AND {$filterObjectTable}_filter.filter = {$this->adapter->quote($filter)} OR ";
                }
            ...
            $result[$filterable['class']] = array_unique($this->adapter->fetchCol($sqlQuery));
        }
        return $result;
    }
    ...
    public function loadAllIds($filterableData)
    {
        ...
        return $this->adapter->fetchCol($sqlQuery);
    }
    ...
    public function loadFilterCategoryData($filterable, $id)
    {
        ...
        return $this->adapter->fetchRow($sqlQuery);
    }
